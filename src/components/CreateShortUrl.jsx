...
const CreateShortUrl = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://url-shortner-backend-3kqe.onrender.com/urls/shorten', { originalUrl: url } );
      setShortUrl(`https://url-shortner-backend-3kqe.onrender.com/urls/${response.data.shortUrl}`);
      setError('');
    } catch (err) {
